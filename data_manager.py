...
class DataManager:
    def __init__(self):
        ...
        for key in TWELVE_API_KEYS:
            self.key_usage.append({"key": key, "daily": 0, "minute": 0, "minute_window_start": time.time()})
    ...
    def _to_twelve_symbol(self, symbol):
        return {
            ...
            "GBPJPY": "GBP/JPY", "EURGBP": "EUR/GBP", "GOLD": "XAU/USD", "SILVER": "XAG/USD",
        }.get(symbol, symbol)
    ...
    def _choose_twelve_key_index(self):
        ...
        if self.twelve_frozen_for_day or time.time() < self.twelve_pause_until_ts:
            return None
    ...
    def _normalize_twelve_values(self, values):
        return [{"datetime": row.get("datetime"), "open": row.get("open"), "high": row.get("high"),
                 "low": row.get("low"), "close": row.get("close"), "volume": row.get("volume", "0")} for row in values]
    ...
    def _normalize_binance_klines(self, klines):
        ...
        for item in klines:
            out.append({
                ...
                "open": str(item[1]), "high": str(item[2]), "low": str(item[3]), "close": str(item[4]),
                "volume": str(item[5]),
            })
        out.reverse()
        return out
    ...
    def _normalize_finnhub(self, data):
        ...
        for i in range(len(closes)):
            out.append({
                ...
                "open": str(opens[i]), "high": str(highs[i]), "low": str(lows[i]),
                "close": str(closes[i]), "volume": str(volumes[i] if i < len(volumes) else 0),
            })
        out.reverse()
        return out
    ...
    def _normalize_alpha_fx(self, data):
        ...
        for dt_str, row in ts.items():
            out.append({
                "datetime": dt_str, "open": row.get("1. open"), "high": row.get("2. high"),
                "low": row.get("3. low"), "close": row.get("4. close"), "volume": "0"
            })
        return out[:50]
    ...
    def _fetch_binance(self, symbol, interval="1min", limit=50):
        ...
        try:
            r = requests.get("https://data-api.binance.vision/api/v3/klines",
                             params={"symbol": symbol, "interval": binance_interval, "limit": limit}, timeout=10)
            data = r.json()
    ...
    def _fetch_finnhub(self, symbol, interval="1min"):
        ...
        try:
            r = requests.get("https://finnhub.io/api/v1/forex/candle", params={
                "symbol": finnhub_symbol, "resolution": "1", "from": now - 3000, "to": now, "token": FINNHUB_API_KEY
            }, timeout=10)
            data = r.json()
            if data.get("error"):
    ...
    def _fetch_alpha(self, symbol, interval="1min"):
        ...
        if cached:
            ...
            return cached
        if not ALPHA_VANTAGE_API_KEY or self.alpha_frozen_for_day or time.time() < self.alpha_pause_until_ts:
            return None
        if len(symbol) != 6:
            return None
        try:
            r = requests.get("https://www.alphavantage.co/query", params={
                ...
                "interval": "1min", "outputsize": "compact", "apikey": ALPHA_VANTAGE_API_KEY
            }, timeout=12)
            data = r.json()
