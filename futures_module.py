...
class FuturesModule:
    ...
    def execute_signal(self, plan, live=False):
        self.base_url = (self.base_url or os.getenv("BINANCE_FUTURES_BASE_URL", "https://fapi.binance.com")).strip()
        self.api_key = (self.api_key or os.getenv("BINANCE_FUTURES_API_KEY", "")).strip()
        self.api_secret = (self.api_secret or os.getenv("BINANCE_FUTURES_API_SECRET", "")).strip()
    ...
    def _resolve_interval(self, timeframe):
        raw = str(timeframe or "1m").strip().lower()
        mapping = {"1m": "1min", "1min": "1min", "m1": "1min", "5m": "5min", "5min": "5min", "m5": "5min"}
        return mapping.get(raw, "1min")

    def analyze_market(self, market, capital_state=None, asset=None, execution_mode=None, timeframe=None, strategy_name=None, leverage_override=None, risk_pct_override=None, max_trades_per_day=None):
        execution_mode = str(execution_mode or self.default_execution_mode or "paper").lower()
        interval = self._resolve_interval(timeframe)
        timeframe_label = "5 minutos" if interval == "5min" else "1 minuto"
        desired_timeframe_code = interval
        candidates = []
        for item in market or []:
            ...
            item_timeframe = str(item.get("timeframe_code") or "1min").strip().lower()
            if not candles or item_timeframe != desired_timeframe_code:
                candles = self.data_manager.get_candles(symbol, interval=interval, outputsize=120)
            if not candles:
                continue
        ...
        if risk_pct_override not in (None, ""):
            try:
                risk_profile["risk_pct"] = max(0.0025, min(0.02, float(risk_pct_override)))
                risk_profile.setdefault("reasons", []).append("Risco customizado aplicado pelo painel futures")
            except Exception:
                pass
        if leverage_override not in (None, ""):
            try:
                leverage = int(max(2, min(self.max_leverage, round(float(leverage_override)))))
                reasons_override = risk_profile.setdefault("reasons", [])
                reasons_override.append("Leverage customizada aplicada pelo painel futures")
            except Exception:
                pass
        ...
        if max_trades_per_day not in (None, ""):
            try:
                plan_limit = int(float(max_trades_per_day))
                if plan_limit > 0:
                    reasons.append(f"Limite diário configurado: {plan_limit} trades")
            except Exception:
                pass
        ...
        plan = {
            ...
            "setup_type": str(strategy_name or "futures_confluence"),
            "strategy_name": str(strategy_name or "futures_confluence"),
